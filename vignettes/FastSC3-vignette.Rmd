...
train <- exprs(shekhar)[fData(shekhar)$fsc3_features, ]
colnames(train) <- pData(shekhar)$cell_type
train <- train[order(rownames(train)), ]

study <- exprs(macosko)[fData(macosko)$fsc3_features, ]
colnames(study) <- pData(macosko)$cell_type
study <- study[order(rownames(study)), ]

res <- SC3:::support_vector_machines(train, study, "linear")
labs_orig <- names(res)
labs_new <- as.character(res)
plot_sankey(labs_orig, labs_new)
irr::kappa2(cbind(labs_orig, labs_new))$value

res <- random_forest(train, study)
labs_orig <- colnames(study)
labs_new <- as.character(res)
plot_sankey(labs_orig, labs_new)
irr::kappa2(cbind(labs_orig, labs_new))$value
...
train <- exprs(macosko)[fData(macosko)$fsc3_features, ]
colnames(train) <- pData(macosko)$cell_type
train <- train[order(rownames(train)), ]

study <- exprs(shekhar)[fData(shekhar)$fsc3_features, ]
colnames(study) <- pData(shekhar)$cell_type
study <- study[order(rownames(study)), ]
